...
public class SubtractionFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = lf.inflate(R.layout.fragment_subtraction, container, false);

        SharedPreferences highscores = getActivity().getSharedPreferences("highScores", Context.MODE_PRIVATE);
        String[] easyArray = highscores.getString(EndResultActivity.HIGHSCORE_SUBTRACTION_EASY, "x/xZx/0").split("/xZx/");
        String[] hardArray = highscores.getString(EndResultActivity.HIGHSCORE_SUBTRACTION_HARD, "x/xZx/0").split("/xZx/");
        double pointsEasy = Double.parseDouble(easyArray[1]);
        double pointsHard = Double.parseDouble(hardArray[1]);
        TextView easyTextView = (TextView) view.findViewById(R.id.subtractionEasyScoreTextView);
        TextView hardTextView = (TextView) view.findViewById(R.id.subtractionHardScoreTextView);
        if(pointsEasy != 0) {
