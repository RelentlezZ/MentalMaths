...
public class AdditionFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = lf.inflate(R.layout.fragment_addition, container, false);

        SharedPreferences highscores = getActivity().getSharedPreferences("highScores", Context.MODE_PRIVATE);
        String[] easyArray = highscores.getString(EndResultActivity.HIGHSCORE_ADDITION_EASY, "x/xZx/0").split("/xZx/");
        String[] hardArray = highscores.getString(EndResultActivity.HIGHSCORE_ADDITION_HARD, "x/xZx/0").split("/xZx/");
        double pointsEasy = Double.parseDouble(easyArray[1]);
        double pointsHard = Double.parseDouble(hardArray[1]);
        TextView easyTextView = (TextView) view.findViewById(R.id.additionEasyScoreTextView);
        TextView hardTextView = (TextView) view.findViewById(R.id.additionHardScoreTextView);
        if(pointsEasy != 0) {
