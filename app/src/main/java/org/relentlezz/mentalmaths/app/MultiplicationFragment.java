...
public class MultiplicationFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = lf.inflate(R.layout.fragment_multiplication, container, false);

        SharedPreferences highscores = getActivity().getSharedPreferences("highScores", Context.MODE_PRIVATE);
        String[] easyArray = highscores.getString(EndResultActivity.HIGHSCORE_MULTIPLICATION_EASY, "x/xZx/0").split("/xZx/");
        String[] hardArray = highscores.getString(EndResultActivity.HIGHSCORE_MULTIPLICATION_HARD, "x/xZx/0").split("/xZx/");
        double pointsEasy = Double.parseDouble(easyArray[1]);
        double pointsHard = Double.parseDouble(hardArray[1]);
        TextView easyTextView = (TextView) view.findViewById(R.id.multiplicationEasyScoreTextView);
        TextView hardTextView = (TextView) view.findViewById(R.id.multiplicationHardScoreTextView);
        if(pointsEasy != 0) {
